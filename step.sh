...
$THIS_SCRIPT_DIR/upload.js "${upload_path}" "${gateway_url}" "${qyrus_username}" "${qyrus_password}" "${qyrus_team_name}" "${qyrus_project_name}" "${qyrus_suite_name}" "${app_activity}" "${device_pool_name}" "${upload_app}" "${enable_debug}" "${bundle_id}" "${email_id}" || $?